...
def simulate(twin: Twin, bundle: RandomBundle, scenario: dict | None = None) -> Simulation:
    s=scenario or {}
    known={r['rule_id'] for r in twin.model['rules']}
    invalid=set(s.get('cancel_rule_ids',[]))-known
    if invalid: raise FDTError('UNKNOWN_RULE','취소 대상 rule_id가 없습니다.',{'rule_ids':sorted(invalid)})
    ...
    for j,f in enumerate(twin.model['components']):
        multiplier=1-s.get('expense_reductions',{}).get(f['envelope'],0) if f['kind']=='expense' and not f['protected'] else 1
        flows[:,:,j]=_scaled(flows[:,:,j],multiplier)
    for rule in twin.model['rules']:
        if rule['rule_id'] in s.get('cancel_rule_ids',[]): continue
        f=twin.model['components'][rule['component']]
        factor=s.get('income_multiplier',1) if f['kind']=='income' else s.get('expense_multiplier',1) if f['kind']=='expense' else 1
        for i,values in bundle.scheduled[rule['rule_id']]:
            flows[:,i,rule['component']]+=values
            calendar.append({'date':bundle.dates[i].isoformat(),'event_type':'RECURRING_'+f['kind'].upper(),
                'reference_id':rule['rule_id'],'account_id':f['account_id'],'card_id':f['card_id'],
                'expected_amount_krw':round(float(np.mean(values))*factor),'source':rule['source'],
                'timing_basis':'purchase_date' if f['card_id'] else 'estimated_cash_date'})
    for j,f in enumerate(twin.model['components']):
        factor=s.get('income_multiplier',1) if f['kind']=='income' else s.get('expense_multiplier',1) if f['kind']=='expense' else 1
    ...
    for j,f in enumerate(twin.model['components']):
        values=flows[:,:,j]
        kind=f['kind']
        if kind in ('income','reimbursement'): resource_delta+=values
        elif kind not in ('internal_transfer','card_settlement'): resource_delta-=values
        if kind=='expense':
            spending+=values
            k=ENVELOPES.index(f['envelope'])
            by_env[:,:,k]+=values
            if f['budgeted']: budgets[:,:,k]+=values
        if not ready: continue
        if f['card_id']:
            policy=cards[f['card_id']]
            ai=account_lookup[policy['settlement_account_id']]
            if kind!='expense': raise FDTError('UNSUPPORTED_CARD_FLOW',kind)
            ...
            if kind in ('income','reimbursement'): daily_cash[:,:,ai]+=values
            elif kind=='internal_transfer':
                daily_cash[:,:,ai]-=values
                daily_cash[:,:,account_lookup[f['to_account_id']]]+=values
            else: daily_cash[:,:,ai]-=values
    if ready:
        ...
        for (card_id,due),vals in sorted(bill_calendar.items()):
            if np.any(vals):
                calendar.append({'date':due,'event_type':'CARD_BILL','reference_id':card_id+'@'+due,
                    'account_id':cards[card_id]['settlement_account_id'],'card_id':card_id,
    ...
    for e in s.get('cash_events',[]):
        ...
        resource_delta[:,offset]+=sign*e['amount_krw']
        if sign<0:
            spending[:,offset]+=e['amount_krw'];by_env[:,offset,-1]+=e['amount_krw'];budgets[:,offset,-1]+=e['amount_krw']
        # Even history-only scenarios cannot silently refer to an unknown account.
        observed_ids={t.account_id for t in twin.transactions if t.account_id}|set(ids)
        if e['account_id'] not in observed_ids:
            raise FDTError('SCENARIO_ACCOUNT','cash_event의 계좌를 확인할 수 없습니다.')
        if ready: daily_cash[:,offset,account_lookup[e['account_id']]]+=sign*e['amount_krw']
        calendar.append({'date':e['date'],'event_type':'SCENARIO_'+e['direction'],'reference_id':'cash_event',
                        'account_id':e['account_id'],'card_id':None,'expected_amount_krw':e['amount_krw'],
    ...
    if ready:
        ...
        if np.any(payable<0): raise FDTError('PAYABLE_INVARIANT','카드 미결제액이 음수가 되었습니다.')
        free=total-payable
    ...
    return Simulation(bundle.dates,resource,spending,by_env,budgets,cash,total,payable,free,ids,
                      sorted(calendar,key=lambda c:(c['date'],c['event_type'],c['reference_id'])),s)
